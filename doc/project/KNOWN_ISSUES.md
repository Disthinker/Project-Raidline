...
| RL-CONTENT-002 | 现有 Pistol/Rifle 共用单一 9mm 弹药，弹匣按具体弹药 ID 兼容，无法形成口径、普通弹档位和职责配装梯度 | 当前切片建立 CaliberDefinition、按口径装填、六职责武器与三口径两档普通弹；保留所有旧 ID 和冻结 pending Raid，不重写既有射击手感 |
...
| Content Beta：完整目录开发覆盖 | PR #117 的 content v53 已随 `8164e66` 进入 main；当前 Macro 2 将同一幂等机制扩展到 content v54 的 50 种定义。它便于直接测试所有内容，但会削弱正式成长和来源梯度；Macro 3 前必须裁决保留为开发选项、缩减为代表性起始包或移除。当前不重复发放、不覆盖既有位置，满仓零修改且不锁档 |
| Content Beta：防具、胸挂与背包梯度 v1 | 当前 `codex/content-beta-loadout-gear-v1` 已实现轻/均衡/重三档取舍、定义驱动容器 UI、24×16 Stash 和 v53→v54 兼容；本地 1317/1317 CTest 通过，尚待 Draft PR、exact-head CI 和用户正常游玩验收 |

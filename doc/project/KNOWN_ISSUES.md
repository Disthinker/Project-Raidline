...
- PR #148 已按用户授权合入 `2ae898a`。首局整备—撤离—回营再出发已在主线；当前独立分支只审查下一基地防守阶段，不含新玩法代码。
...
- 当前 Base 敌人更新和离散事实同步保存只为有限外围规模设计。实时防守不能直接放大该循环，也不能嵌入 Raid 生命周期来绕过持久化；下一计划须明确导航预算、战斗检查点与唯一防守结算。
- 安全核心不受敌人侵入、个人资产不因外围死亡丢失、Base 即时整备均仍有效。让核心可受攻城、增加新的财产/人口损失或改变中断恢复，必须以新的产品合同明确决定，不能由“继续开发”静默推导。
...
- schema v45 不承诺旧程序读取；回滚前保留升级前 v44 安全备份，不用真实玩家档案做迁移测试。正常新游戏仍是覆盖确认流程；验收新档前自行保留原档，代理不会改玩家存档。

...
| RL-COMBAT-008 | 距离同时缩放并占用散布包络，遮蔽移动/甩枪/击发变化；移动叠加鼠标输入时准星可能闪动 | PR #71 第一轮已拆分独立 Bloom、改为连续投影与像素对齐并通过 CI；用户确认原问题基本解决。第二轮正移除轻微甩动硬台阶、放慢展开/恢复、增强横向后坐力，并加入默认 8% 有效射程 Distance Bloom |
...
| Combat：动态散布模型与准星稳定性 v3 | PR #71 第一轮 CI 已通过；第二轮已完成连续移准阈值、较慢时序、小权重距离 Bloom、F10 参数和增强横向后坐力，147 项定向回归与 778/778 CTest 通过；最终 CI 与人工验收待完成 |

...
- **RL-PERF-BASE-HIT-001（2026-09-09）**：基地周边每次命中同步全档写入，已用真实磁盘复现约
  29 ms 峰值；旧 Raid 测试未配置存储且不要求实际命中，漏掉 Daily 热路径。当前独立 stacked 修复
  `base-perimeter-hit-checkpoints` 去掉逐命中写盘与无关 Profile 复制，新增真实存储/错误/连射/退出门禁；
  尚待精确 CI 和用户正常游玩，不提前写成主线已修复。
- Daily 改为每 250 ms 有界检查点；正常退出必须成功排空，写失败或持久落后 1 秒时显式暂停并重试。
  异常终止只恢复最近完整检查点；大型 Profile 的完整存储工作仍存在，只是离开命中帧，慢存储可能触发保护。
  本片不删校验、不改存档格式、不承诺所有硬件/无限物品量无延迟，也不启动地图/导航重写。
- #160 工事内部检查点暂停；本修复因 schema 47 兼容显式依赖它，不自动合并任何一个 PR。

- 2026-09-09 最新覆盖：#149–#157 已合入，#159 计时测试串行隔离修复也已合入 `main@c6d4597`。
  当前工事分支只达到领域/持久化内部检查点，不具备可发布的安装、拆障或自动防守收益。
  不在 B 目录出售未完成工事，不要求用户验收不可见功能。不能把候选位置局部净空等同于全局防守合法性。
- 工事完整交付的剩余门禁是合法安装与 v2 预警冻结、实时失效清理、自动磨损与同场恢复、建设交互。
  旧 Defense v1 和 Raid 回滚保持，不以该切片恢复 Daily 全局寻路、NPC 守军或正式美术。

以下为历史状态，不覆盖上方最新记录：

- **当前裁决（2026-09-09）**：#156@`f16544d` 集成版本已获用户验收，精确双平台 CI 通过，本次全量回归 1697/1697。RL-LIFECYCLE-001 及后续已复现的配置/反馈、接触/保护、帧顺序、Profile/空间切换问题已在集成分支解决；主线 `2ae898a` 尚不包含这些修改。
- 本次收尾未发现需要新增代码的阻断项；不表示全项目零缺陷。Daily 真实寻路、全局 Profile/combat 帧事务、固定步长和大型文件拆分仍是有明确开启条件的保留项，不能混写为已完成或默认下一任务。详见 [收尾评审的工程债与门禁](../architecture/GAMEPLAY_FRAMEWORK_CLOSEOUT.md)。
- #149–#156 为未合并依赖链；#149 单独是存在已知问题的历史基线，不能单独作为合格发布版本。下一步先取得合并授权并收束整条交付链，不继续增加架构层或新玩法。

### 历史发现与各切片当时状态

- 当前覆盖（2026-09-09）：#155 时序统一已验收、精确双平台 CI 通过，未合并。本轮 `combat-runtime-boundaries-v1` 已复现并修复同进程读档沿用旧外围缓存，以及出击/回营/换周期的旧弹道残留；原实现还会保留空间清理后的击发标志。修正属于明确的生命周期行为变化，不宣称纯代码移动。
- 保留：同一防守事件的完整检查点恢复、普通同周期状态同步、每个 Raid 空间的存活/死亡与附属状态。未合并活动循环或 Profile 保存政策；同一场防守恢复不是应清空弹道的“新活动”。正常游玩与精确 CI 状态见本轮 PR，不继承 #155 验收。
...
- 入侵停滞回归已覆盖：不可达位置连续枪声令同一波次放弃防线目标；贴墙合法位置被导航的额外 2 单位间距判为非法起点。防守专用导航现与物理足迹一致；枪声仍产生警觉，但不可见目标不替换防线目标，可见暴露玩家仍可被追击/攻击。普通 Raid/外围听觉规则不变，安全核心/公共软失败/个人资产边界不变。分支修复仍待用户正常游玩确认。
- PR #149 用户返工：预警按钮未接入 Base 点击路由，且 warningActive 每帧强制显示无关闭状态，导致亲自/自动防守及倒计时后的选择均不可操作。开发分支已接入独立预警意图路由与共享按钮矩形；Esc/X 收起、F6 或有指针时点击提示条重开。收起是会话表现状态，不取消事件/倒计时或已有自动预设，不写存档。新增真实 SDL 事件队列的无窗口回归；仍待用户正常游玩复验，未合入主线。
- PR #148 已按用户授权合入 `2ae898a`。首局整备—撤离—回营再出发已在主线；PR #149 已获方案确认，实施玩家实时基地防守，尚未通过用户正常游玩验收。
- 职责配装匹配不等于当前可击发；新供弹提示严格区分 `Fired`、仅上膛、故障、损坏和无供弹。告警仍可二次确认出击，不强制防具或医疗。
- Base/结果页关闭提示会保存偏好；Raid 内 H 只隐藏本次运行提示，不保存活动 Raid。异常关闭仍精确恢复出击前档案，提示也可能恢复；真实结果确认回营后永久结束首次指导。
- PR #148 已修复旧 pending Raid 恢复到空结果页的问题，保留恢复诊断但直接回 Base；不生成失败结果或额外损失。
- PR #149 实时防守使用独立有限敌人运行时、既有空间索引/导航配额与 250ms 合批检查点，不嵌入 Raid 生命周期。普通外围敌人在本次事件内冻结并隐藏，结束恢复原状态，不重复生成 Loot 或计算外围返程耗时。
- SafeCore 不受侵入；标记缓冲走廊仅供当前事件交战。6 次防线突破、玩家倒下或二次确认放弃只产生既有公共软失败，个人物品原位保留，消耗不返还。正常退出恢复同一防守，强制结束可能整体回退未保存尾段；超过 1000ms 未持久窗口或保存失败暂停保护，不能宣称逐发零回退。Raid 的出击前回滚不变。
- 相关性只表示公开物资来源的权重分档，不保证本局掉落；无物资情报显示未知。战术地图不标记实际物品、不绘制导航。
- 带回贡献统计成功撤离时全部合法随身相关资产（包括出击前携带物），不是本局净新增产量；不会移动或自动捐献。跨周期失效仍保留资产。
- 新档建家前暂停唯一 WorldClock，人口/岗位/设施/经营压力尚不存在；临时物资点仅访问同一 Stash。建立后恢复已有时钟/外围与经营，不能理解为所有 Home Region 永久无危险。
- 三个地块当前只用程序图形和文字区分空间，不添加地区数值增益；固定障碍不能移动。旧档不进入选址，不搬动已有物品或设施。选中地块上有地面物时须先收起，避免建立设施埋住物品。
- PR #149 写 schema v46，读取旧 v45 不生成虚构防守事件；旧程序不承诺读取新档。回滚前保留升级前 v45 备份，不能用已升级的滚动备份冒充旧版本备份。测试均使用临时目录，不修改真实玩家存档；验收新档前自行保留原档。
...
| RL-BASE-013 | 聚合岗位占位只能查看，点击作业点不能直接进行岗位分配、清除或自动补员 | PR #140 已让 Workshop/Medical 作业点进入现有设施检查器并提供岗位专用右键菜单；经 CI 与用户验收后以 `0f74a67` 进入 main，逐人 NPC 与排班继续延期 |
| RL-BASE-014 | 宿舍床位只有通用作业点图形，基地世界无法直接读取聚合居民、伤病、床位与劳动力占用 | PR #141 已建立只读居民空间投影，并让 DormitoryBunk 提供一致的状态、宿舍入口和自动补员；经 CI 与用户验收后以 `2f05626` 进入 main，不新增逐人居民状态 |
| RL-BASE-015 | 仓库装卸点和厨房处理点只有通用图形，基地世界无法读取统一库存、自动供给、资源池与预计短缺 | PR #142 已建立只读供给准备度和设施物资流投影，并让两个稳定作业点提供对应功能入口；经 CI 与用户验收后以 `b1194a1` 进入 main，不建立第二套库存或逐件物流模拟 |
| RL-BASE-016 | 五类作业点始终显示完整状态文字，缩远时信息拥挤；运营总览也缺少短缺/居民压力与筛选分页 | PR #143 已建立缩放信息层级、状态分类、筛选分页和点击定位，经 CI 与用户正常游玩验收后以 `60c83fa` 进入 main |
| RL-BASE-017 | Home Region 基地外仍是无风险空场景，统一大地图缺少本地外围探索闭环 | PR #144 已建立安全核心/过渡缓冲/外围、有限感染者、低阶 Loot 和保留资产的返回/救回结果，经 CI 与用户正常游玩验收后以 `674f65e` 进入 main；不嵌入 Raid 生命周期且不提供导航 |
| RL-BASE-018 | 单一指定物品愿望无法随人口形成多目标取舍，也不能组合不同物品贡献或预览浪费 | 当前分支升级为按周期冻结人口的一至三个四类愿望、显式多资产贡献与超额预览；schema v43 保持 v42 当前单愿望事实且不重抽，等待完整验证和用户正常游玩验收 |

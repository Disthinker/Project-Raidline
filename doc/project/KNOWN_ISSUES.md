...
| RL-COMBAT-008 | 距离同时缩放并占用散布包络，遮蔽移动/甩枪/击发变化；移动叠加鼠标输入时准星可能闪动 | PR #71 已拆分独立 Bloom、连续投影与像素对齐；第二轮 head `13176d9` exact-head CI 通过。第三轮新增走路/奔跑独立即时扩散、提高持续甩枪展开并适度加快恢复，149 项定向回归与 780/780 CTest 通过，等待最终 CI 与人工验收 |
...
| Combat：动态散布模型与准星稳定性 v3 | PR #71 第二轮 head `13176d9` CI 已通过；第三轮完成走路/奔跑即时扩散、F10 奔跑参数、快速甩枪展开与适度恢复，149 项定向回归与 780/780 CTest 通过；最终 CI 与人工验收待完成 |

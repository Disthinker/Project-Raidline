...
| RL-CONTENT-003 | 新增武器、弹药、防具和容器缺少清晰来源梯度，固定供应与制造也未覆盖三口径基础恢复 | 当前 Macro 3 已实现定义驱动固定供应、三张固定图与三类高危危机独立 Loot 表、三口径标准弹制造和制造回收防套利；Windows Debug 与 1324/1324 CTest 通过，尚待 exact-head CI 和用户正常游玩验收 |
...
| Content Beta：战利品来源、经济与补给可持续性 v1 | 当前分支已完成固定地图/高危危机产出梯度、定义驱动固定供应、三口径标准弹制造与防套利校验；本地 1324/1324 通过，尚待 exact-head CI 和用户正常游玩验收 |

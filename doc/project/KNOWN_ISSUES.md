...
| RL-BASE-015 | 仓库装卸点和厨房处理点只有通用图形，基地世界无法读取统一库存、自动供给、资源池与预计短缺 | 当前分支已建立只读供给准备度和设施物资流投影，并让两个稳定作业点提供对应功能入口；Windows Debug、83/83 定向回归和 1427/1427 全量 CTest 通过，待 CI 与用户正常游玩验收；不建立第二套库存或逐件物流模拟 |

...
| RL-COMBAT-011 | 相对鼠标模式的实际准星与控制目标可持续移动到当前屏幕外，玩家可能无法找回准星 | Draft PR #111 当前分支允许权威准星和目标越出当前屏幕约 48px（约一个当前准星直径），再由外扩边界清除外向速度；反向输入立即生效，世界边缘仍是硬边界。Windows Debug 与当前 1255/1255 CTest 通过，待 exact-head CI 和用户正常游玩验收 |
...
| RL-PERF-003 | Frontier Exchange 中持续移动或移动准星时会规律性回退数毫秒，形成高帧率下仍可见的顿挫 | 用户 F9 证据为 Event/Update/Render-Present/Pacing/Total MAX `0.22/69.24/5.34/13.72/73.04 ms`。真实 Frontier 冻结地图回归稳定复现 `69.97～71.58 ms` Update 峰值，确认根因是 Active Raid 每秒复制并校验包含完整冻结布局的 Profile，而非导航或显示提交；当前分支改为原位推进已验证活动状态，连续十次回归最慢 Update 降至 `0.59～0.76 ms`，世界时间与 Profile 合法性保持正确。待 exact-head CI 和用户正常游玩复验 |
...
| RL-MAP-009 | `Frontier Exchange` 的已接受主线仍是 1280×720 小型随机掩体样例，缺少可支撑完整 Raid 的超大主题分区、内容密度和可扩展投影/导航 | Draft PR #111 当前分支已建立 25600×14400 layout v3、八个连续分区、三个固定地标、约束道路/环境/玩法 Socket、schema v35 冻结、区块投影和分层导航；128-seed 与 100 敌人定向回归通过，完整 CI 和用户三 seed 正常游玩验收待收束 |
| RL-MAP-010 | layout v3 车辆、设备、集装箱和建筑仍沿用小地图尺寸，且中心格占位允许大型矩形相互重叠；同步生成时缺少明确加载反馈 | Draft PR #111 当前分支已按角色尺度冻结真实长宽原型，除碎屑外以完整矩形唯一占位并避让玩法锚点；128 个 seed 验证道路物件数量、可达性和两两无重叠。进入 layout v3 前先提交双语加载画面；Windows Debug 与 1255/1255 CTest 通过，待 exact-head CI 和用户正常游玩验收 |
...
| 程序化 Raid 内容扩展：完整室外布局生成 v3 | 当前分支把 Frontier 扩展为 25600×14400 单主题无缝室外地图，以八区、固定地标、冻结道路/地表/环境/玩法 Socket、区块投影、分层导航和确定性 v3 回退替代旧整图背景；完整资源点生态与遭遇生态仍由后续宏切片承接 |

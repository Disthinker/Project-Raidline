...
- **当前裁决（2026-09-09）**：用户验收和合并授权后，#149–#157 已普通合入 `origin/main@c3eb19c`，完整树等于已验收版本。RL-LIFECYCLE-001 及后续已复现的配置/反馈、接触/保护、帧顺序、Profile/空间切换修复现已在主线。本次本地全量 1697/1697；合并 head CI 见当前状态与 PR。
...
- 后续 [固定防御位/基础路障评审](../exec-plans/active/base-defense-fixed-positions-v1.md) 尚未实现。已确认工程差距：现有主设施布局不能直接表示重复工事；移动阻挡同时挡弹道；旧防守几何不可变；工事损伤不能套用玩家保护/伤势。不得靠永久无敌矩形、每击全世界重建或伪造 Enemy 死亡结果绕开这些合同。
- 外部 GDD 防守文档仍含早期“未实现”标题及废止的两支四人组/逐发 NPC 弹药描述；本轮只读，不能据此恢复 NPC 守军范围。新工事若要改变 SafeCore、个人损失或布局冻结，须独立产品决策。

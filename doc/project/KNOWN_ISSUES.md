...
| RL-PERF-004 | 在超大 Frontier 地图拾取物品会复制并重复校验冻结 Profile，产生可见单帧卡顿 | 当前 Macro 3 分支把 Raid 拾取和局内库存事务收窄到 AssetRegistry 参与者；完整 Profile 校验保留在部署、保存、恢复和结算边界。Windows Debug 性能回归由约 69.6 ms 降至约 2.8 ms，并覆盖过期 revision 时 Loot 标记和 Profile 指纹零修改；待完整 CI 与用户正常游玩复验 |
...
| RL-AI-002 | 超大地图 36～48 名初始敌人仍是独立随机散点，路线、资源与地标没有形成可理解的遭遇压力，且初始出生点附近可能立即生成敌人 | 当前 Macro 3 分支以 content v48/schema v37/rules v26 冻结巡逻、地标守点、路线伏击和同组声响响应，并强制初始群组避开玩家出生点周围 1200 世界单位；128 个固定 seed 已验证。复用既有视线/搜索/寻路预算，不扩张为新 AI 大阶段，待完整 CI 与用户正常游玩验收 |

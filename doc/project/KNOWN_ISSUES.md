...
| RL-COMBAT-008 | 距离同时缩放并占用散布包络，遮蔽移动/甩枪/击发变化；移动叠加鼠标输入时准星可能闪动 | `codex/combat-spread-model-v3` 已拆分三种独立 Bloom、改为连续投影与像素对齐；130 项定向回归和 778/778 全量 CTest 通过，CI 与人工验收待完成 |
...
| Combat：动态散布模型与准星稳定性 v3 | 当前分支已完成距离包络与独立 Bloom、连续准星投影、130 项定向回归和 778/778 CTest；PR/CI 与人工验收待完成 |

...
最后核对：2026-08-09。本文是当前问题、风险和延期债务的仓库内权威记录；GitHub Issue 用于跟踪可执行项，源码、测试和 CI 仍是行为事实来源。
...
| RL-W26-001 | P2 | 瞄准 UX | Week26 原 1–10 人工验收通过后发现代码准星与系统鼠标同时显示。App 已在活动 Raid、库存关闭且准星可用时隐藏系统鼠标，并在菜单、Base、库存、终局和 shutdown 恢复。 | Closed | 用户确认补充 11–13 通过；feature head `3a52354` 的 Windows/Ubuntu CI 通过并由 PR #48 合入。 |
| RL-W26-002 | P2 | 投射物表现 | 白色方块与 700 px/s 原型先改为 900 px/s 火光拖尾，再按反馈缩为 3×3 热芯与两段细线；最终提高到 1200 px/s，以最多 8 px 常规步长解析高速命中，并将灰色粒子改为 16 枚短寿命白热/金黄/红橙火花。逻辑 AABB 仍为 8×8、伤害仍为 1。 | Closed | 用户确认最终验收 15 通过；全量测试、精确 feature head Windows/Ubuntu CI 与 PR #48 合入完成。 |
...
| RL-INV-003 | P2 | 弹药堆叠 | 用户于 2026-08-09 报告：弹药可被 Ctrl/Shift 拆分为多堆，但把同定义弹药拖到另一堆时无法合并。该现象违反既有“同定义非满堆优先合并、9mm 单堆上限 60”的领域合同，说明真实窗口拖拽到已占用格的路径缺少覆盖或发生回归。 | Open | 在仅玩家背包和双容器场景复现并修复；拖放到同定义堆时先补至 60，剩余数量采用冻结后的原子放置/保留规则，稳定 ID 与总数量不丢失；补 App 层回归测试并完成真实窗口验收。 |
| RL-COMBAT-001 | P2 | 命中反馈 | Week29 当前 fallback 会在任意敌人命中时显示准星 X；用户于 2026-08-09 修订为普通命中不显示准星命中反馈，只有爆头或命中弱点时显示专用反馈。当前尚无命中部位/弱点判定，不能仅在渲染层猜测命中类型。 | Needs Decision | 战斗命中部位任务冻结弱点定义、爆头判定与反馈样式；领域命中结果显式携带命中类型，普通命中不触发准星标记，爆头/弱点只触发一次专用反馈，并覆盖误报、连续命中和非敌人碰撞。 |
| RL-COMBAT-002 | P2 | 肢体破坏与打击感 | 产品方向：步枪爆头可破坏整个头部并产生血液飞溅；爆炸物或霰弹枪可按确定规则随机打断四肢，同时产生身体后退、断肢碎块击退与强烈受击表现。当前没有命中部位、武器伤害标签、可破坏身体状态或碎块生命周期。 | Needs Decision | 独立战斗表现 ExecPlan 冻结哪些命中可断肢、致死/非致死规则、各武器权重、可注入随机源、击退与碰撞、血腥表现开关、对象上限和资源合同；自动测试保证确定性与状态唯一性，真实窗口验证可读性和性能后关闭。 |
| RL-COMBAT-003 | P2 | 尸体生命周期 | 产品方向：敌人死亡后应在场景中留下尸体；后续尸体还需能承接头部/四肢破坏结果。尸体是否整局保留、是否碰撞/可搜索、数量上限、分层与跨局清理尚未冻结。 | Needs Decision | 尸体系统任务冻结所有权与生命周期；死亡只生成一次尸体表现，尸体不继续参与 AI/攻击/伤害，Raid 重置时清理，并在确定的保留时长或数量上限下通过多敌人性能与遮挡验收。 |
| RL-COMBAT-004 | P2 | 弹道与地面反馈 | 产品方向：击发瞬间快照准星的世界坐标，弹丸以该快照为固定落点，之后移动鼠标不得改变弹道；弹丸到达未被敌人/障碍提前拦截的落点时产生地面粒子。当前弹丸按射击方向飞到命中或世界边界，没有目标点和地面到达语义。 | Needs Decision | 弹道任务冻结物理弹丸/视觉曳光的语义、敌人和障碍提前拦截、最大射程、越界/过近目标处理及地面材质反馈；测试覆盖准星快照不变、固定落点、提前命中仅结算一次和地面粒子仅生成一次。 |

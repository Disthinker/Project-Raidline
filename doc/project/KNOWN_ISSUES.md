...
最后核对：2026-08-11。本文是当前问题、风险和延期债务的仓库内权威记录；GitHub Issue 用于跟踪可执行项，源码、测试和 CI 仍是行为事实来源。
...
| RL-INV-003 | P2 | 弹药堆叠 | 普通整堆拖拽原先只走 transform/transfer，已占用的同定义弹药格始终判为非法；独立分支现已统一普通拖拽查询/提交，目标补到上限 60，容量不足时源堆余量原地保留，精确数量拖拽合同不变。 | Local Fixed | Windows Debug 全目标重建和全量 CTest 552/552 已通过；用户于 2026-08-11 确认真实窗口同/跨容器、溢出、非法目标和精确数量回归 1–5 全部通过。待精确提交 Windows/Ubuntu CI 通过后关闭。 |

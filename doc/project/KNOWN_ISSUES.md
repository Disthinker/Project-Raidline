...
- 医院室内镜头被室外条件禁用，已移除该限制；未购图只显示购买提示的旧行为按用户要求
  改为移动局部黑图。室内进出、地图信息裁剪与镜头边缘仍需本修复的正常游玩复验。

- **2026-09-13 医院 Draft #164**：真实 seed 4 暴露群组最少人数高于随机总人数，保留医院
  总人数 24～32，仅将巡逻组最低人数放宽为 2；加入内容兼容校验，旧 pending 继续冻结结果。
  房间标识/携物撤离/重开用药/两条寻回进入回归；正式 CI 与用户正常游玩仍为独立门禁。

- **2026-09-12 验收后覆盖**：#162 已由用户验收，普通合并至 main **2ae1a8c**，精确双平台
  1795/1795 全通过。下方“尚无人工验收”属于历史。安装确认的空间验证/保存成本仍存在，
  本片不承诺零延迟；正式美术、NPC、自由筑墙和导航扩展继续排除。

- **2026-09-12 覆盖**：#160 已按用户授权普通合入 main 7ee5f27。收尾 #162 已补齐日常工事
  实体/弹道/残架投影及 B 购买/储备、固定位置预览、换位/收纳/维修；尚无完整工事人工验收。
  不变预览复用缓存，安装确认仍有一次完整空间验证和保存成本，不承诺零延迟；工事变化
  不重建敌人。预警允许 B 维修，不允许购买/位置变化，实时交战不开放建设。
  需用户观察所有缩放档的文字/固定位置识别与真实拆障节奏；自动化不能替代正常游玩证据。

- **2026-09-11 自动防守覆盖**：冻结来向的工事抵扣/30 耐久磨损及双语预览已接通，
  下文“自动磨损未实现”保留为历史。当前剩余是 B 工事目录、鼠标安装/收纳/维修与
  Daily 阻挡投影；购买仍关闭，尚非完整玩家交付。没有正式资源或保存格式扩张。

- **2026-09-11 最新覆盖**：本轮接通预警一次冻结和 Profile 耐久写回，schema 48 新事件 Defense v2；
  下文“生产拒绝 v2”现为上一步历史。旧事件继续 v1，禁止降级新警报/活动到旧格式。
  自动贡献/磨损、B 界面及日常工事阻挡/投影仍未接通，购买不得开放；此处不是工事完整验收。
  新警报的一次空间准备仍有成本，但不在命中/逐帧路径执行；完整大档后台写盘能力仍受现有
  250ms 合批/1秒持久落后保护约束，不承诺零延迟或无限规模。

- **2026-09-11 优先覆盖**：#160 已接入工事拆障/失效与动态阻挡，内部 runtime v2 检查点
  已能恢复攻击绑定和损坏缺口；以下 9 月 10 日“尚未受损”描述保留为历史。
  仍未完成预警冻结和 Profile 签出/耐久写回，所以生产开始/读档拒绝内部 v2，B 购买仍关闭。
  自动防守磨损和建设 UI 不得绕过此门禁。runtime JSON 往返不等于完整玩家存档已验收。
  静态导航只在准备/恢复构建，至多四件动态工事即时覆盖；完好绕路须通过准备证明。
  不因此恢复 Daily 寻路、全局 Session 重构或正式美术。

- **2026-09-10 当前覆盖**：RL-PERF-BASE-HIT-001 已由用户验收，#161@f394058 双平台
  1725/1725 通过，普通合入 #160 分支 `b179ac9`；main 仍未包含该修复，不能混淆交付层级。
- #160 恢复：安装空间证明与原子位置事务进入内部实施，B 购买仍关闭。
  建设查询只返回稳定身份与合法性；确认时重查当前世界/资产，不信任旧绿框或任意 App 矩形。
  保守通路窗口可能拒绝必须绕到很远才能到达的旧异常布局，不移动既有物品来强行通过。
- **剩余真实风险**：尚未接入路障碰撞/受损/一次失效、v2 冻结快照、自动磨损或可游玩建设界面。
  内部安装 API 不构成可发布工事，不提供单独人工验收；后续 UI 不得逐帧重复全通路证明，
  应按 Profile/空间版本缓存只读预览，释放时再重验。旧 Defense v1 不受本内部能力影响。

以下为 2026-09-09 当时记录，不覆盖上述最新接受状态：

...
| Content Beta：完整目录开发覆盖 | Draft PR #117 的 content v53 会向新档和首次读取该版本的旧档一次性补齐全部当前已发布物品；便于本阶段直接测试所有内容，但会削弱正式成长和来源梯度。Macro 3 前必须裁决保留为开发选项、缩减为代表性起始包或移除；当前不重复发放、不覆盖既有位置，满仓零修改且不锁档 |

...
| RL-MAP-009 | `Frontier Exchange` 仍是 1280×720 的小型随机掩体技术样例，缺少可滚动的大地图、可理解道路骨架、街区阻挡及完整冻结/回退证据 | 当前 `codex/procedural-raid-layout-v1` 已建立 2560×1440 layout v2、主/次/接入道路、70～95 个建筑阻挡、schema v34 冻结与镜头坐标投影；自动化、PR/CI 和用户正常游玩验收收束中 |
...
| 程序化 Raid 内容扩展：完整室外布局生成 v1 | 当前分支把 Frontier 扩展为可滚动的 2560×1440 单主题室外大地图，以冻结道路/建筑、合法区和确定性回退替代旧整图背景；资源点生态、遭遇生态和既有消费者全面整合由后续宏切片承接 |

...
| RL-COMBAT-011 | 相对鼠标模式的实际准星与控制目标可持续移动到当前屏幕外，玩家可能无法找回准星 | Draft PR #111 当前分支已将权威准星和目标限制在 48px 内缩的可见世界矩形，边缘清除外向速度且反向输入立即生效；Windows Debug 与 1252/1252 CTest 通过，待 exact-head CI 和用户正常游玩验收 |
...
| RL-PERF-003 | Frontier Exchange 中持续移动或移动准星时会规律性回退数毫秒，形成高帧率下仍可见的顿挫 | 用户 F9 证据为 Event/Update/Render-Present/Pacing/Total MAX `0.22/69.24/5.34/13.72/73.04 ms`，已排除单纯显示提交节奏；当前首要嫌疑为超大稠密导航网格上的同步寻路。尚未修复，下一切片需用 Update 分段观测锁定并消除峰值 |

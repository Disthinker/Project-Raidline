...
- 玩家：WASD 移动、朝向保持、斜向归一化、Space 连续射击和冷却；Week24 分支已接入默认 3 HP 与调试文本。
- 敌人与战斗：水平移动和边界反弹、Health、AABB 命中、伤害、死亡、调试分数；Week24 分支中敌人接触可按冷却伤害玩家并触发真实失败结算。
...
- Raid 与撤离：自动开始 180 秒 Raid；玩家中心进入左下撤离区并连续停留 3 秒可撤离，离开会清零进度；撤离、接触伤害导致的玩家死亡或超时形成不可逆终局并冻结世界玩法更新。
...
尚未实现：可操作 Stash UI、从 Stash 选择出战物、跨进程保存、局内重开、多柜体/搜索计时、外部数据驱动 Loot、治疗、击退/受伤表现、敌人攻击 AI、装备栏、上下文菜单、重量/耐久、音频、复杂 AI、程序地图、尸体搜索和完整武器装填/弹匣系统。Week24 接触伤害只闭合 V0 失败循环，不代表完整战斗系统；新 Raid 使用空背包，也不代表配装或持久化已经完成。9mm 只是数量物品，不代表完整武器弹药系统已经完成。

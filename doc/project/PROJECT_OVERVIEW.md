...
尚未实现：完整 Raid 生命周期、撤离、结算、Stash、多柜体/搜索计时、外部数据驱动 Loot、装备栏、上下文菜单、重量/耐久、保存读取、音频、复杂 AI、程序地图、尸体搜索和完整武器装填/弹匣系统。当前搜索状态只在单个 GameplayWorld 生命周期内存在，9mm 也只是数量物品，不代表完整武器弹药系统已经完成。

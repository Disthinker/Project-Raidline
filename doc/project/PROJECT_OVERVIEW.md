...
- 可重复会话（Week23 已通过 PR #42 合入）：结算后显示只读 Stash 网格，按 `N` 以空背包开始下一 Raid；世界、柜体、敌人和倒计时重置，Stash 与稳定 ID 序列在当前进程内跨局保留。

## 当前产品边界

尚未实现：可操作 Stash UI、从 Stash 选择出战物、跨进程保存、玩家 Health/战斗死亡接线、局内重开、多柜体/搜索计时、外部数据驱动 Loot、装备栏、上下文菜单、重量/耐久、音频、复杂 AI、程序地图、尸体搜索和完整武器装填/弹匣系统。Week23 只承诺当前进程内的第二局、只读 Stash 与稳定 ID 连续性；新 Raid 使用空背包，不代表配装或持久化已经完成。9mm 只是数量物品，不代表完整武器弹药系统已经完成。

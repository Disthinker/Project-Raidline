...
- 本地 Week22 结算：撤离把玩家背包完整堆叠原样转入内存 Stash，死亡/超时清空携带物并记录损失；结算反馈显示栈数和单位数。该能力尚待真实窗口与 CI 验收，未进入 `main`。

## 当前产品边界

尚未实现：Stash 操作 UI、出战选择、跨进程保存、玩家 Health/战斗死亡接线、第二局/局内重开、跨 Raid 稳定 ID 分配、多柜体/搜索计时、外部数据驱动 Loot、装备栏、上下文菜单、重量/耐久、音频、复杂 AI、程序地图、尸体搜索和完整武器装填/弹匣系统。当前 GameplayWorld 与搜索状态仍只有一个 Raid 生命周期，Week22 Stash 也只在当前 App 进程内存在；9mm 只是数量物品，不代表完整武器弹药系统已经完成。

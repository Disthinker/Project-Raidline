...
尚未实现：最终主菜单/基地美术与功能、地图副本选择、鼠标瞄准射击与后坐力、敌人抓/挠/咬攻击、敌人感知/追击/攻击决策 AI、可操作 Stash UI、从 Stash 选择出战物、跨进程保存、局内重开、多柜体/搜索计时、外部数据驱动 Loot、治疗、击退/受伤表现、装备栏、上下文菜单、重量/耐久、音频、程序地图、尸体搜索和完整武器装填/弹匣系统。Week25 只是代码绘制的单地图流程壳，不代表基地系统或多地图内容完成；Week24 接触伤害只闭合 V0 失败循环，不代表完整战斗系统；新 Raid 使用空背包，也不代表配装或持久化已经完成。9mm 只是数量物品，不代表完整武器弹药系统已经完成。

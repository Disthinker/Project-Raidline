...
- 背包：玩家 10×6 与第二容器 6×6、确定性 first-fit、footprint 占用、稳定实例 ID、事务式放置/移动/跨容器转移。
- 背包 UI：Tab 开关；纯鼠标 hover、单击选择和阈值拖拽；平滑虚像与吸附 footprint 分离；Esc 取消；方向键和 Enter 无背包语义。
- 物品丢弃：玩家物品可拖到明确的丢弃区并在角色脚下生成地面物品；第二容器物品不能直接丢弃。

## 当前产品边界

尚未实现：完整 Raid 生命周期、撤离、结算、Stash、可搜索地图容器与 Loot table、装备栏、物品旋转、快捷转移、上下文菜单、堆叠/重量/耐久、保存读取、音频、复杂 AI、程序地图、尸体搜索和完整武器弹药系统。当前第二容器只是 Week18 的内存内交互载体，不代表地图搜刮生命周期已完成。

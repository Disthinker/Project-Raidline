...
Core Extraction Alpha 到 Regional Operations 基础阶段、“首张可玩随机大地图”阶段、Content Beta 成长闭环、Home Region 外围探索、基地愿望 v2 和远征研判（PR #146 / `f92df3e`）均已进入主线。当前唯一切片是“开局选址与首座基地建立 v1”，合同见 `doc/exec-plans/active/home-region-founding-v1.md`，外部 GDD 继续只读。

近期顺序：本片连续完成安全考察、三个固定局部地块、唯一核心建立、原有建设与整备/首次出击衔接；自动化/CI 后集中正常游玩验收。下一步先根据试玩检查三地块空间差异、初始物资点说明、第一次整备/出击的可读性，再与策划确定下一宏切片；不直接扩张实时守城、第二主题、导航、任务板或正式美术。

...
Core Extraction Alpha 到 Regional Operations 基础阶段、“首张可玩随机大地图”阶段、Content Beta 成长闭环，以及 Home Region 聚合岗位世界占位反馈均已进入主线。当前推进设施岗位现场操作 v1，范围合同见 `doc/exec-plans/active/home-region-workforce-world-actions-v1.md`，外部 GDD 继续只读。
...
| Home Region 设施岗位现场操作 v1 | 直接从 Workshop/Medical 作业点完成选择、分配、清除和自动补员 | 稳定作业点命中、类型化岗位动作过滤、既有持久化命令、无 schema/content 迁移 | 当前开发分支；受影响目标、147/147 定向回归与 1421/1421 全量 CTest 已通过，等待 exact-head CI 与用户正常游玩验收 |

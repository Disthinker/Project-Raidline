...
当前覆盖：用户已验收 #155@c0b0664，活动切片改为 `active/combat-runtime-boundaries-v1.md`，独立 stacked 依赖 #155，均未合并。收拢成功读档/新档、出击/回营、外围周期和防守/室内切换的空间状态失效边界；保留同场恢复和现有活动 Policy。

下一具体任务是**玩法框架整合收尾评审**：汇总生命周期、敌人配置/反馈、导航选择、接触保护、帧顺序和空间边界的实际调用；核对依赖 PR 的顺序、累计回归与验收缺口，给出是否可结束重构的结论。不因旧计划曾列出 Session/Persistence 阶段就继续扩写，也不自动恢复新玩法或合并。

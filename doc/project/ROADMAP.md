...
| Macro 3：遭遇与部署生态 v1 | 不同路线出现巡逻、地标守点、近距伏击和同组声响响应压力 | 内容驱动群组原型、合法分区/地标外围锚点、冻结成员与巡逻点、schema v37/content v47、现有有预算 AI 运行时适配 | 当前分支实现；不增加新敌人类型，不开启行为树、派系、AI 队友或普通阶段无限增援 |

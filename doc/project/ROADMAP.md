...
当前窄切片为攻击接触与伤害保护合同，显式依赖 #153@d70f861（精确 head 双平台 CI 通过、人工验收待完成、未合并）。计划见 `active/enemy-contact-protection-v1.md`：三环境共享接触有效性、Grab/Bite 消费和 0.25 秒保护，隔墙和被保护接触不产生伤害/控制。保留活动策略，不升级存档，不启动新玩法。

下一门禁是三环境正常游玩复验与依赖栈收尾；随后只评审同帧“玩家击杀/敌人命中”顺序的真实差异，补充时序合同并提出最小变更。若统一为敌人先推进/攻击，必须另列为行为变更，而不是自动做行为保持重构。完成时序裁决后再据剩余重复决定是否抽取组合边界；Session/Persistence 整体重写不是默认任务。

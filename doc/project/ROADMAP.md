...
Core Extraction Alpha 到 Regional Operations 基础阶段、“首张可玩随机大地图”阶段、Content Beta 成长闭环、Home Region 外围探索、基地愿望 v2、远征研判和开局选址（PR #147 / `805fd8b`）均已进入主线。当前唯一切片是“首局整备—撤离—回营再出发 v1”，合同见 `doc/exec-plans/active/first-raid-onboarding-v1.md`，外部 GDD 继续只读。

近期顺序：本片连续完成供弹真实就绪、首次地图风险、成功/失败回营与第二次整备的上下文指导；自动化/CI 后集中正常游玩验收。下一步具体检查新档“考察建家→配装供弹→真实 Raid 终局→原位整理/损失恢复→第二次出击”，只修复该完整链的阻塞和可读性回归，通过后关闭 Home Region 起步阶段。再向策划提交下一大阶段范围，实时守城仅为候选而非当前授权；不直接扩张第二主题、导航、任务板、AI小队或正式美术。

...
| 代表性枪械与口径梯度 v1 | 轻型、通用和远距/重型配装在操控、射程、供弹与穿甲上形成可感知差异；开发阶段可直接检查全部当前发布物品 | 六职责武器、三口径、两档普通弹、领域兼容图、旧 ID/旧 pending Raid 兼容、content v53 一次性全目录 Stash 发放 | Draft PR #117；当前分支 `codex/content-beta-weapon-caliber-v1`，从 `origin/main@e285a2b` 开始；完整目录覆盖会削弱成长节奏，Macro 3 前必须复核其发行形态 |

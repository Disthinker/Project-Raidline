...
- 修复同定义弹药拖放到已占用堆时无法按上限 60 合并的回归，并补真实窗口/App 层覆盖（RL-INV-003）。
...
- 命中部位与弱点：普通命中不显示准星确认，只对爆头/弱点提供专用反馈（RL-COMBAT-001）。
- 肢体破坏与高强度打击表现：按武器和命中部位驱动头部/四肢破坏、血液、身体及碎块击退，并设置可控的性能与表现边界（RL-COMBAT-002）。
- 敌人尸体及破坏状态残留，包含确定的生命周期、数量上限和 Raid 清理规则（RL-COMBAT-003）。
- 击发时冻结准星世界落点，弹丸到达该点时生成地面命中粒子，同时保留敌人/障碍提前拦截语义（RL-COMBAT-004）。

这些方向相互依赖，推荐先建立“命中结果携带命中部位 + 击发快照携带目标点”的领域合同，再实现尸体/肢体状态，最后集中接入血液、碎块和专用反馈资源。它们尚未进入 Week29 冻结范围，也不会在正式美术暂停期间自动启动。

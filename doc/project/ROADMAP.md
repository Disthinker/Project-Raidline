...
| Raid 往返行动耗时 v1 | 三张图显示不同抵达时间；出发、正常返程和失败归队推进世界时钟，异常退出精确回滚 | MapDefinition travel、冻结活动快照、schema v9、幂等时间/需求 Settlement | 当前分支实现与自动化中；不包含夜间视野、路线状态、旅行遭遇、哨所、精力或睡眠 |

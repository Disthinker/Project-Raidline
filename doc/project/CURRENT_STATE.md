...
- 当前切片增加一个可购买、可选择合法位置放置、可持久化并参与空间碰撞的基地储物箱，继续复用唯一 AssetRegistry 与 BaseGround 容器访问事务。
...
Core Extraction Alpha 到 Regional Operations 基础阶段、“首张可玩随机大地图”阶段、Content Beta 成长闭环、Home Region 有限新档整备、大地图、共享战斗、地面资产与世界容器访问均已进入主线。当前切片加入可购买、可在基地板块内选择位置放置并参与空间碰撞的持久储物箱；继续复用唯一 AssetRegistry、BaseGround 与世界容器事务，不改变 Raid、Settlement 或存档 schema。
...
68. **Home Region 可放置储物箱 v1（当前开发分支）**：新增固定供应储物箱与类型化世界占地；玩家在基地板块内通过鼠标预览、旋转并原子放置，非法边界和重叠位置拒绝，放置后参与移动/弹道阻挡并复用世界容器界面。

...
65. **Home Region 共享战斗能力 v1（当前开发分支）**：提取 Base/Raid 共用的 SDL-free 瞄准、散布、后坐力、逻辑弹道与表现投影；基地将支持真实武器切换、弹药/耐久消耗和故障，但保持无敌人、无撤离、无 Settlement。
...
- PR #122 已通过 Windows Debug、1344/1344 CTest、exact-head Windows/Ubuntu CI 和用户正常游玩验收，以 `62e464e` 进入 main。当前共享战斗切片已完成 SDL-free `WorldShootingRuntime`、Base 真实武器事务与客户端输入/表现接入；Windows Debug 全目标和 1347/1347 完整 CTest 通过，exact-head CI 与用户正常游玩验收待完成。开发代理未启动游戏。

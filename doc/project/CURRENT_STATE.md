...
10. **准星运动、逻辑弹道与开发调参 v1**：位置/速度/加速度准星、刷新式后坐力、稳定性随机散布、五项武器属性、最大距离弹道、基础障碍/弱曳光、基础开镜、奔跑举枪、射程反馈和 F10 运行时调参正在 PR #67 形成完整玩家闭环。
...
- PR #67 旧角度限速实现曾以提交 `92f6d94` 通过 734/734 和 exact-head CI，但用户尚未验收即修订了手感合同；该旧证据不作为当前代码接受证据。新版实现已在 Visual Studio Developer Shell、x64 host/x64 target 下完成 Windows Debug 全目标构建、174 项射击 focused tests 和全量 CTest 742/742，开发代理未启动游戏。
...
- `WeaponAimState` 保存实际准星世界位置、鼠标目标位置、玩家控制速度和后坐力速度；鼠标突然从 A 转向 B 时，准星从当前 P 按最大速度与人机工效派生加速度向 B 运动。玩家朝向、准星表现和成功击发都消费同一个实际准星，而不是原始鼠标点。
- 击发按“枪口到实际准星”方向刷新一份有界后坐力初速度并加入少量 PCG32 横向偏转；再次击发替换旧后坐力速度而非叠加，随后按人机工效派生反向加速度减到零。准星位置不会被自动拉回旧点。
...
- 准星运动、逻辑弹道与开发调参 v1：PR #67 新版实现已完成本地构建、focused tests 和全量 CTest 742/742；exact-head Windows/Ubuntu CI 与用户正常游玩验收尚待完成。

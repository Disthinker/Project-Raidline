...
- 实际准星只确定总体射击方向；每发在当前散布圆锥内使用确定性 PCG32 偏移冻结最终方向。精准度控制最小散布，稳定性控制最大散布及射击/快速移准增长，操控速度控制停火收缩；最小/最大包络随准星距离平滑增长，贴脸接近零。v3 中距离主要定义上下文包络，并以默认 8% 的有效射程 Bloom 小幅直接贡献当前扩散；移动、快速甩动与击发仍分别形成独立有界 Bloom，外显准星预读同一权威散布而不额外改变子弹随机结果。

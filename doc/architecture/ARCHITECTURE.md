...
- 程序化室外 layout v3 先生成八个连续主题分区，再放置三个固定内部地标、4/3/2 格道路、压缩地表段、环境物件和全部玩法 Socket。分区、地标、道路、地表、物件、锚点、生成尝试、回退原因与完整哈希共同进入 pending Raid；达到尝试上限只能使用同一份已验证的 v3 固定回退骨架，不能保留半合法结果、退回旧背景或在加载后重抽。GameplayWorld 建立 16×16 格区块只读表现索引，SDL client 只消费相机附近投影；导航使用静态障碍索引、局部环形搜索、区块门户与局部 A*。屏幕/世界坐标转换仍只属于表现输入边界，不进入领域真值。
...
- `assets/content/v1/core.json` 是物品、容器、装备、经济、地图、敌人/Loot、基地、区域节点/路线、基地候选点、地点独特设施、稳定设施和轻量哨所配置的单一内容输入；当前开发内容版本为 `procedural-frontier-district-layout-content-44`。程序化室外加载额外验证 layout v3、320×180 基础格、20×12 区块、八个分区实例、三个地标模板、道路/物件/水洼密度和生成预算；稳定 ID、清剿/前哨引用、迁徙/设施和既有内容约束继续统一生效。CMake 配置时压缩行空白、分块为合法编译器字符串并嵌入只读生产代码。
...
- Persistent Base 落地 schema v1，Extraction Loop、Survival Loadout、Base Growth、Raid World 和 Regional Operations 逐步演进；当前开发 schema v35 在 v34 layout v2 快照之上保存 layout v3 的分区、地标、地表、环境物件和玩法锚点。schema v34/layout v2 保留原冻结数据并按旧合同读取，不用 v3 重生成；更早档案继续沿既有迁移链补齐旧布局/威胁/区域/设施状态。内容版本兼容仍独立于 Profile schema。
...
23. `codex/procedural-raid-layout-v1`：当前主阶段 Macro 1，交付 25600×14400 超大主题分区地图、layout v3 分区/地标/道路/地表/环境/玩法 Socket、确定性回退、区块投影、分层导航、schema v35/content v44 与代码占位表现。

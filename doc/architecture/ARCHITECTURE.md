...
- `BaseDefenseCheckpointWriter` 是服务层单用途写入器：一个写入线程、一个可合并候选，250ms 请求一致检查点；磁盘失败或未持久时间超过 1000ms 暂停活动。开始、结算及退出等待持久化屏障，不在击发/逐敌伤害回调同步保存。
- 活动命令与每帧伤害加入同一资产/战斗检查点；伤害热路径只暂存健康、伤势和护甲参与者，外部命令、保存和载入仍做全档校验。拒绝模拟事务不得保存“已命中但未扣弹”的半帧，重试恢复最后完整持久检查点。
...
- Regional Operations、首张随机大地图、Content Beta 及 Home Region 起步阶段已收束；当前 PR #149 实施已获确认的有限玩家实时防守。长期系统只保留当前消费者需要的边界；NPC 守军、工事、AI 小队或联机状态不得空转创建。
...
- `BaseRuntime` 只保存玩家位置、Home Region 确定性室外布局、可见区投影、碰撞、设施交互范围、稳定 FacilityId 和短期交互上下文。安全基地是 Home Region 中的固定内部结构板块；外围环境不会因此获得 Raid 的敌人、Loot、撤离或 Settlement 权威。权威 WorldClock、居民、设施、资产和区域地点继续保存在 ProfileState。Base 与 Raid 可组合同一个窄 `WorldShootingRuntime` 以复用瞄准、散布、后坐力、逻辑弹道和只读表现投影，但不得因此共享敌人、RaidSession、Loot 或 Settlement 生命周期。
...
设施、人口、任务、区域节点/路线/基地候选点/哨所和 NPC 使用各自稳定 ID 与领域状态，不塞入物品 AssetRegistry。基地候选点持有长期解锁与独特设施修复事实，以及内容定义的迁徙容量/耗时和独特设施效果；唯一 `TechnologyCoreState` 与 active Base 必须指向同一地点。`BaseConstructionState` 唯一保存已拥有设施的 Installed/Reserve 位置和进入储备的世界分钟，设施储备不占 Stash；重装时按该时间戳整体平移相关队列，使储备期间不损失剩余进度。轻量哨所只持有解锁/建立、聚合驻守、中断状态和自上次恢复以来的捷径行动计数；任何地点均不拥有第二套 Stash、资产、世界时钟或服务队列。清剿与恢复目标作为 Pending Raid 的强类型快照存在，不让 UI 通过地图名猜测任务。
...
- 区域路线在 Deploy 时冻结；一次 Settlement 对同一哨所最多累积一次捷径威胁，即使路线含多个受该哨所门控的边。达到内容阈值只影响结算后的后续查询，不改写本局冻结返程。哨所恢复与基地候选点清剿均由 GameSession 把“初始敌人已清除”投影为统一撤离资格位，Simulation 不读取 Profile，SDL client 不决定长期解锁或恢复成功。主基地迁徙使用独立纯查询与原子命令：推进内容定义时间和日结，冻结设施队列剩余进度，迁移唯一科技核心，切换 active Base，并把新旧地点的主基地/哨所角色互换；Stash、资产和人口不复制。地点独特设施使用另一条纯查询/原子命令，可在当前主基地或已建立前哨的已解锁候选点预先修复；修复事实永久属于地点，实际效果只在该地点是 active Base 时由明确消费者读取。
...
`HomeFoundingState` 属于 Profile，不引入第二套基地或物品容器。`home_founding_domain` 保存三候选的稳定 Plot 定义，提供纯查询及原子建立命令；`GameSession` 负责候选保存，`GameFlow` 用已选 Plot 配置互斥的 BaseWorld。普通新游戏明确启动 survey，原有已建立工厂函数保留为旧档/领域测试基线。建立前唯一时钟暂停，不补扣并不存在的基地需求；建立后恢复既有消费者。SDL 只显示地块/确认与可关闭提示，不执行设施预算或所有权规则。
...
- 程序化室外 layout v3 先生成八个连续主题分区，再放置三个固定内部地标、4/3/2 格道路、压缩地表段、环境物件和全部玩法 Socket；layout v4 在同一稳定布局上增加普通、高价值和地点专属资源点。资源点以内容定义声明容量、Loot 表、风险、合法分区或地标绑定，Deploy 使用独立命名随机流冻结实例、点内槽位和实际 Loot。分区、地标、道路、地表、物件、锚点、资源点、生成尝试、回退原因与完整哈希共同进入 pending Raid；达到尝试上限只能使用同一份已验证的固定回退骨架，不能保留半合法结果、退回旧背景或在加载后重抽。GameplayWorld 建立 16×16 格区块只读表现索引，SDL client 只消费相机附近投影；导航使用静态障碍索引、局部环形搜索、区块门户与局部 A*。屏幕/世界坐标转换仍只属于表现输入边界，不进入领域真值。
...
- Persistent Base 落地 schema v1，Extraction Loop、Survival Loadout、Base Growth、Raid World 和 Regional Operations 逐步演进；当前开发 schema v36 在 v35/layout v3 快照之上保存 layout v4 资源点实例及每个 Raid Loot 的来源点和点内槽位。schema v35/layout v3 与 schema v34/layout v2 均保留原冻结数据并按旧合同读取，不用 v4 重生成资源点；更早档案继续沿既有迁移链补齐旧布局/威胁/区域/设施状态。内容版本兼容仍独立于 Profile schema。

...
- Raid 目标模拟步长为 60 Hz；渲染与模拟分离仍需后续完整迁移。当前生产 App 已先把单次帧时间限制为 100 ms，避免失焦、远程桌面或慢帧触发无界追帧；敌人导航按空间与角色尺寸预计算静态可见图、以 10 Hz 刷新动态目标，并限制每子步昂贵查询数量。数量级回退由 `doc/architecture/PERFORMANCE_BUDGETS.md` 的 Debug 压力门槛阻断。

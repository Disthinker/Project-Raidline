...
- Base 与 Raid 共用同一按住拖拽内核和领域预览；正常游玩即可完成整理、配装、压弹、装匣、指定换弹、Raid 弹匣卸弹和治疗，不存在只为验收设置的动作按钮。
...
- Interaction：共享 4 像素阈值、抓取点、旋转、数量锁定、Tab/Esc 仲裁和 revision 失效；Profile 格子/装备/弹药动作查询与提交保持原子；Raid 卸弹覆盖可发现性、随身目标选择、中断零提交和完成时弹药守恒。
...
2. Raid 内通过右键弹匣完成一次卸弹，并完成换弹、治疗、Loot 整理和撤离。

...
- Base 与 Raid 共用同一按住拖拽内核和领域预览；正常游玩即可完成整理、配装、压弹、装匣、指定换弹和治疗，不存在只为验收设置的动作按钮。
...
4. **玩家反馈返工：统一库存与角色显示**
   ...
   - 4 像素阈值后才开始拖动；原物不动、虚像跟随，绿色/蓝色/红色和动作文字共同显示查询结果。Ctrl/Shift 数量在按下时锁定，旋转/取消/窗口失焦/revision 过期均不提交。
...
- Interaction：共享 4 像素阈值、抓取点、旋转、数量锁定、Tab/Esc 仲裁和 revision 失效；Profile 格子/装备/弹药动作查询与提交保持原子。

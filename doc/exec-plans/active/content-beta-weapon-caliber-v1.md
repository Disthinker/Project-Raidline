...
用户于 2026-08-31 将本切片的开发可用性边界修订为：新档及首次读取本内容版本的旧档，Stash 一次性获得当前 `ContentRegistry` 中全部已发布物品定义。可堆叠物按单堆上限发放，非堆叠物每种一件；既有资产不删除、不搬位。该发放由稳定事务 `bootstrap.warehouse_catalog.content_53` 幂等记录，不是每日供应、商店补货或 Raid 奖励。仓库空间不足时整笔零修改，但旧档仍可进入 Base，清理空间并重启后再尝试。它是开发阶段的完整目录可访问覆盖，会削弱新档成长节奏；Macro 3 的来源/经济切片必须重新评估是否保留、缩减或迁移为开发选项。
...
- content v53；schema 保持 v38，因实例保存结构不变；rules 保持 v28，因本切片只向现有语义 Loot 表补充定义、不改变冻结快照结构与抽取规则。content v52、v51 与更早受支持版本继续加载。
- 新档在初始 Profile 构造时直接形成完整目录；旧档只在 pending Raid 回滚完成后执行一次原子补齐并立即保存。已经存在的普通满堆或非堆叠定义不会重复创建；救济批次不冒充普通仓库库存。
...
4. 接入 Base 供应与受控来源：三种标准普通弹和空弹匣进入固定基础供应；新增武器与增强普通弹只进入现有工业、地点专属或高危 Loot 表。正式风险梯度重排留给 Macro 3。退出条件：高级内容不进入无限基础供应或救济。
5. 更新双语检视/配装警告，完成自动化、Windows Debug、完整 CTest、exact-head Windows/Ubuntu CI，再交给用户正常游玩验收。
6. 按用户修订补齐完整已发布目录：新旧档共享同一原子发放函数，旧档迁移幂等，容量不足不锁档。该步骤不改变后续物品的正常 Loot、供应、消费、失败和寻回合同。
...
- [x] 实现口径/弹药定义与兼容查询。
- [x] 迁移装填、击发穿甲消费和双语内容投影。
- [x] 发布六职责武器、三口径两档普通弹与弹匣。
